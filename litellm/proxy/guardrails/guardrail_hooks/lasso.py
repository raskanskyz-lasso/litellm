...
class LassoGuardrail(CustomGuardrail):
    def __init__(self, api_key: Optional[str] = None, **kwargs):
        ...
        self.deputies = os.environ.get("LASSO_DEPUTIES")
        if not self.api_key:
            msg = (
        ...
        self.api_base = os.environ.get("LASSO_API_BASE") or "https://server.lasso.security"
    ...
    async def call_lasso_guardrail(self, data: dict, hook: str) -> None:
        ...
        headers = {
            ...
            "lasso-litellm-hook": hook
        }

        # Prepare request payload and endpoint
        prompt = data.get("messages", [])

        # Build the endpoint with query parameters if deputies are provided
        query_params = {}
        if self.deputies:
            query_params["services"] = self.deputies
        endpoint = f"{self.api_base}/gateway/v1"
        if query_params:
            endpoint += "?" + urlencode(query_params, doseq=True)
